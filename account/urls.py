...
urlpatterns = [
    ...
    # JWT authentication
    path('login/jwt/', CustomTokenObtainPairView.as_view(),
    ...
    # Password management
    path('password/reset/', UserViewSet.as_view(
        {'post': 'request_password_reset'}), name='password_reset_request'),
    path('password/reset/confirm/', UserViewSet.as_view(
        {'post': 'reset_password_confirm'}), name='password_reset_confirm'),
    path('password/change/',
         UserViewSet.as_view({'post': 'change_password'}), name='password_change'),
]
